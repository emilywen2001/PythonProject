...
jieba.load_userdict(
    "D:\\学习\\大三\\大三上\\现代程序设计技术\\课程资料\\dataset\\stopwords_list.txt")
...
def stopwordslist():
    ...
    stopwords = [line.strip() for line in open(
        "D:\\学习\\大三\\大三上\\现代程序设计技术\\课程资料\\dataset\\stopwords_list.txt", encoding='utf-8').readlines()]
    return stopwords
...
def count(wordlist):
    ...
    # 保留词频大于100的为特征词,并创建字典
    wordcountfilter = {k: v for k, v in wordcount.items() if v > 100}
...
def get_gc_comment(column, dis_list):
    """
    根据向量中心获取最近和最远的弹幕
...
if __name__ == '__main__':
    with open("D:\\学习\\大三\\大三上\\现代程序设计技术\\课程资料\\dataset\\弹幕数据\\danmuku.csv", 'rt', encoding='gbk', errors='ignore') as file:
        reader = csv.reader(file)
        # 获取初始弹幕文本
        column = [row[0] for row in reader][1:100000]
        ...
        get_gc_comment(column, dis_list)
